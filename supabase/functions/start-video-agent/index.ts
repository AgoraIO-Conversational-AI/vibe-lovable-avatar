...
function buildTtsConfig(key: string, voiceId: string) {
  return {
    vendor: "elevenlabs",
    params: {
      key,
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Avatar config
    const AVATAR_VENDOR = Deno.env.get("AVATAR_VENDOR") || "anam";
    const AVATAR_API_KEY = Deno.env.get("AVATAR_API_KEY") || "";
    const AVATAR_ID = Deno.env.get("AVATAR_ID") || "";
