...
export function useAgoraVideoClient() {
  ...
  const agentVideoUidRef = useRef<string>("");
  ...
  const joinChannel = useCallback(
    async (config: VideoClientConfig) => {
      ...
      try {
        ...
        agentVideoUidRef.current = config.agentVideoUid;

        // Listen for agent audio AND video
        rtcClient.on(
          "user-published",
